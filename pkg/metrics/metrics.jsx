...
class CurrentMetrics extends React.Component {
    ...
    onVisibilityChange() {
        ...
        // try privileged channel so system administrator can see per-service IO usage of all users
        if (!cockpit.hidden && (this.privileged_channel === null)) {
            this.privileged_channel = cockpit.channel({ superuser: "try", payload: "metrics1", source: "internal", interval: 3000, metrics: PRIVILEGED_METRICS });
    ...
    onPrivilegedMetricsUpdate(event, message) {
        ...
        if (!Array.isArray(data)) {
            this.cgroupNames = data.metrics[0].instances;
            return;
        }
        ...
        const topServicesDiskIO = n_biggest(5, this.cgroupNames, this.privilegedSamples[0], this.privilegedSamples[1]);
        newState.topServicesDiskIO = topServicesDiskIO.map(([name, read, write, isUser, isContainer, uid]) => {
                    ...
                    <CardBody>
                        ...
                        { this.state.topServicesDiskIO.length > 0 &&
                            <Table
                                ...
                                cells={ [{ title: _("Service"), transforms: [cellWidth(50)] }, _("Read"), _("Write")] }
                                rows={this.state.topServicesDiskIO}
                                rowWrapper={ props => <RowWrapper cgroup-name={ props.row[0] } {...props} /> }>
                                <TableHeader />
                                <TableBody className="pf-m-tabular-nums" />
                            </Table> }
                    </CardBody>
                </Card>
